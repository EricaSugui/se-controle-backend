...
function adicionarMesesData(dataISO, meses) {
  const data = new Date(dataISO);
  data.setUTCMonth(data.getUTCMonth() + meses);
  return data.toISOString().slice(0, 10);
}

function adicionarMesesCompetencia(competencia, meses) {
...
function mesesEntre(competenciaInicio, competenciaFim) {
  ...
  if (indiceMesIni === -1) {
    throw new Error(`competencia inválida: ${competenciaInicio}`);
  }
  if (indiceMesFim === -1) {
    throw new Error(`competencia inválida: ${competenciaFim}`);
  }
...
module.exports = { adicionarMesesData, adicionarMesesCompetencia, competenciaParaData, mesesEntre };
