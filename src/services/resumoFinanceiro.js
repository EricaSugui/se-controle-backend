...
async function calcularResumo(competencia) {
  ...
  gastosRows.forEach((row) => {
    gastosPorCasa[row.casa_id] = Number(row.total);
  });
  ...
  for (const casa of casasRows) {
    ...
    const { rows: percentualRows } = await pool.query(
      `SELECT percentual FROM percentuais_custeio
       WHERE casa_id = $1 AND competencia = $2
       LIMIT 1`,
      [casa.id, competencia]
    );
  ...
  return {
    ...
    minha_parte_total: minhaParteTotal,
    saldo: receitasTotal - minhaParteTotal,
    casas,
  };
}
