...
router.get('/', async (req, res, next) => {
  try {
    const { rows } = await pool.query('SELECT * FROM metas ORDER BY id');
    res.json(rows);
  } catch (err) {
    next(err);
  }
...
router.post('/', async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      'INSERT INTO metas (objetivo, valor_atual, meta, falta) VALUES ($1, $2, $3, $4) RETURNING *',
      [objetivo, orNull(valor_atual), orNull(meta), orNull(falta)]
    );
